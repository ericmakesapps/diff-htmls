...
describe('Utils', () => {
    ...
    describe('stripTagAttributes()', () => {
        ...
        it('accept string and return stript tag - without any attirubutes', () => {
            expect(Utils.stripTagAttributes('')).toBe('>');

            expect(
                Utils.stripTagAttributes(
                    '<span class="bold" style="font-size: 8px;" id="id-1" >'
                )
            ).toBe('<span>');
            expect(
                Utils.stripTagAttributes('<img src="./image-1.png" />')
            ).toBe('<img/>');
        });
    });
    ...
    describe('wrapText()', () => {
        ...
        it('create tag with passed text inside', () => {
            expect(Utils.wrapText('text', 'span', '')).toBe(
    ...
    describe('stripAnyAttributes()', () => {
        ...
        it('if passed tag - strip attirbutes otherwise return same string', () => {
            expect(Utils.stripAnyAttributes('<tag atribute="value" />')).toBe(
                '<tag/>'
            );
